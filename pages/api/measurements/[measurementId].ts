...
import { getSourceMeasurementByID, updateSourceMeasurement, deleteSourceMeasurement } from '@/api/services/sourceMeasurementService'
  ...
  switch (method) {
    ...
    case ("PUT"):
      try {
        ...
        else res.status(404).json({ error: 'measurement not found' })
      } catch (error) {
