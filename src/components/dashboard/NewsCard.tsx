  ...
  return (
    <Card className="rounded-lg border border-gray-700 bg-gray-900 shadow-lg">
      <CardHeader className="pb-3 pt-6">
        ...
        <CardDescription className="mt-2 text-gray-400">{dataSourceName || 'No data source name'}</CardDescription>
      </CardHeader>
      <CardContent className="pb-6 pt-3">
        <p className="text-gray-300">{description || 'No description available'}</p>
      </CardContent>
      <CardFooter className="rounded-b-lg bg-slate-800 px-6 py-3">
        <div className="flex space-x-4 text-sm text-gray-300">
          <div className="flex items-center">
            <span className="mr-1 text-gray-400">Company:</span>
            <span className="text-white">{companyName || 'No company name'}</span>
          </div>
          <div className="flex items-center">
            <span className="mr-1 text-gray-400">Notification Date:</span>
            <span className="text-white">{formattedDate || 'No date available'}</span>
          </div>
          <div className="flex items-center">
            <span className="mr-1 text-gray-400">Trigger Factor:</span>
            <span className="text-white">{triggerFactor || 'No trigger factor'}</span>
          </div>
        </div>
      </CardFooter>
    </Card>
